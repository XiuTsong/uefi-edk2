    ...
    )
{
    ...
    SetMem(gFilePoolAllocated, sizeof(gFilePoolAllocated), 0);
...
{
    EASY_DIR *NewDir = EasyCreateDir(DirName);
    if (NewDir)
        return EASY_SUCCESS;
    return -EASY_DIR_CREATE_DIR_FAILED;
}
...
{
    return EasyCreateFile(FileName);
}